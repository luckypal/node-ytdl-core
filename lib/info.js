...
exports.getBasicInfo = async(id, options) => {
  console.log('options.requestOptions.requestFnc', options);
  if (options.requestOptions.requestFnc) requestFnc = options.requestOptions.requestFnc;
...
const getWatchHTMLPageBody = (id, options) => {
  ...
  return exports.watchPageCache.getOrSet(url, () => requestFnc(url, options.requestOptions).text());
};
...
const getEmbedPageBody = (id, options) => {
  ...
  return requestFnc(embedUrl, options.requestOptions).text();
};
...
const getWatchJSONPage = async(id, options) => {
  ...
  let body = await requestFnc(jsonUrl, reqOptions).text();
...
const getVideoInfoPage = async(id, options) => {
  ...
  let body = await requestFnc(url, options.requestOptions).text();
...
const getDashManifest = (url, options) => new Promise((resolve, reject) => {
  console.log('Dash Manifest', url, options);
  return;
  let formats = {};
